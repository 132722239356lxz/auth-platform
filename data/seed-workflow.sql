...
-- ============================================================
-- Part 0(扩展): 子系统可见权限申请 (审批通过后调用AI决策应可见子系统)
-- 单级审批: 发起申请 → 系统管理员审批(固定审批人admin) → 审批完成
-- applyContent.type = SUBSYSTEM_VISIBILITY, 由 PermissionGrantService 路由到
-- SubsystemVisibilityGrantProcessor 执行 AI 决策 + sys_user_subsystem 赋权
-- ============================================================

INSERT IGNORE INTO wf_definition (id, definition_key, definition_name, description, category, version, status, created_by, create_time, update_time) VALUES
(7, 'SUBSYSTEM_VISIBILITY_APPLY', '子系统可见权限申请', '普通用户申请子系统可见权限，管理员审批通过后由AI决策应可见的子系统并自动赋权', 'SUBSYSTEM', 1, 1, 'admin', '2026-08-13 10:00:00', '2026-08-13 10:00:00');

INSERT IGNORE INTO wf_node (id, definition_id, node_name, node_type, exec_mode, condition_expression, on_condition_fail, approver_strategy, approvers, approver_role, sort_order, timeout_hours, countersign, reject_strategy, create_time) VALUES
(26, 7, '发起申请',        'START',    'SERIAL', NULL, NULL, 'SPECIFIC', NULL,  NULL, 0, 0,  0, NULL,       '2026-08-13 10:00:00'),
(27, 7, '系统管理员审批',   'APPROVAL', 'SERIAL', NULL, NULL, 'SPECIFIC', 'liangxz', NULL, 1, 48, 0, 'TO_PREV', '2026-08-13 10:00:00'),
(28, 7, '审批完成',        'END',      'SERIAL', NULL, NULL, 'SPECIFIC', NULL,  NULL, 2, 0,  0, NULL,       '2026-08-13 10:00:00');

-- ============================================================
-- Part 2(扩展): 审批表单定义（表单绑定流程，前端按 schema 渲染）
-- 新增/修改审批场景只需新增表单定义，无需改动代码
-- ============================================================

INSERT IGNORE INTO wf_form (id, form_key, form_name, definition_key, schema_json, icon, sort_order, status, apply_type, created_by, create_time) VALUES
(1, 'SUBSYSTEM_VISIBILITY', '子系统可见权限申请', 'SUBSYSTEM_VISIBILITY_APPLY', '[{"field":"title","label":"申请标题","type":"text","required":true,"placeholder":"请输入申请标题"},{"field":"reason","label":"申请理由","type":"textarea","required":true,"placeholder":"请描述您需要访问哪些业务子系统、用途是什么，便于审批人与 AI 决策。","maxlength":300}]', 'Key', 1, 1, 'SUBSYSTEM_VISIBILITY', 'admin', '2026-08-13 10:00:00');
