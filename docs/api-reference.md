...
- [3. 系统管理 system-server (9001)](#3-系统管理-system-server-9001)
  ...
  - [3.4 部门管理](#34-部门管理)
  - [3.5 客户端管理](#35-客户端管理)
  - [3.6 子系统 Token 管理](#36-子系统-token-管理)
  - [3.6 授权审计与吊销](#36-授权审计与吊销)
  - [3.7 请求日志查询](#37-请求日志查询)
...
> **职责**: OAuth2 认证授权、用户注册、Token 签发、用户信息端点  
> **说明**: 客户端管理、授权审计、Token 管理等业务 API 已迁移至 system-server (见第3章)
...
> **所属服务**: system-server (已从 auth-server 迁移)  
> **路径前缀**: `/api/clients`  
...
> **职责**: 用户/角色/菜单管理 + 客户端管理 + 授权审计 + 子系统Token + 请求日志 + 意见反馈 + 个人中心  
> **说明**: 客户端管理/授权审计/Token管理/请求日志等 API 已从 auth-server 迁移至本模块
...
GET /api/users/page                       → 分页查询用户         [权限: system:user:list]
GET /api/users/{id}                       → 按ID查询用户          [权限: system:user:list]
POST /api/users                           → 新增用户              [权限: system:user:edit]
PUT /api/users/{id}                       → 修改用户              [权限: system:user:edit]
...
```
GET /api/users/page?keyword=admin&enabled=true&userType=admin&deptId=1&page=1&pageSize=10
```

> 支持多条件组合筛选，所有查询参数均为可选。后端通过反射绑定参数名到 `UserPageQuery`，不可识别的参数会被忽略。

**查询参数 `UserPageQuery`**

| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|------|------|------|
| keyword | String | ❌ | - | 搜索关键词（模糊匹配 用户名/昵称/邮箱） |
| enabled | Boolean | ❌ | - | 状态筛选（true=启用 false=禁用） |
| userType | String | ❌ | - | 用户类型筛选（admin/user/service） |
| deptId | Long | ❌ | - | 部门ID筛选 |
| roleId | Long | ❌ | - | 角色ID筛选（查询拥有该角色的用户） |
| page | int | ❌ | 1 | 页码（从1开始） |
...
```bash
# 基本分页
curl "http://localhost:9001/api/users/page?page=1&pageSize=10" \
  -H "Authorization: Bearer <TOKEN>"

# 组合筛选
curl "http://localhost:9001/api/users/page?keyword=admin&enabled=true&userType=admin&page=1&pageSize=10" \
  -H "Authorization: Bearer <TOKEN>"
```
    ...
    "records": [
      {
        ...
        "createTime": "2025-01-01T12:00:00",
        "lastLoginTime": "2025-01-02T09:30:00",
...
**响应字段 `PageResponse<UserResponse>`**

| 字段 | 类型 | 说明 |
|------|------|------|
| records | List\<UserResponse\> | 当前页数据列表 |
| total | long | 总记录数 |
| page | int | 当前页码 |
| pageSize | int | 每页条数 |

> **空结果**: `total=0` 时 `records` 为空数组 `[]`，不会返回 `null`
...
| email | String | ❌ | - | `"zhangsan@example.com"` |
| phone | String | ❌ | - | `"13800138000"` |
| userType | String | ❌ | admin/user/service，默认 `"user"` | `"user"` |
| tenantId | String | ❌ | 默认 `"default"` | `"default"` |
| deptId | Long | ❌ | - | `1` |
| roleIds | List\<Long\> | ❌ | 不传则自动分配默认角色（编码 `ROLE_DEFAULT`） | `[2, 3]` |

> 🆕 **默认角色机制**: 新增用户时若未指定 `roleIds`，后端会自动查找编码为 `ROLE_DEFAULT` 或 `DEFAULT` 的角色并分配。请确保数据库中已创建默认角色。
...
curl -X POST http://localhost:9001/api/users \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer <TOKEN>" \
  -d '{"username":"zhangsan","password":"123456","nickname":"张三","email":"zhangsan@example.com","userType":"user","deptId":1}'
```
...
> 仅更新传入的非空字段；`password` 为空字符串或未传入则不修改密码；`username` 修改时会检查唯一性

**请求参数 `UserUpdateRequest`**

| 字段 | 类型 | 必填 | 校验规则 | 示例 |
|------|------|------|------|------|
| username | String | ❌ | 3-50字符 | `"zhangsan_new"` |
| password | String | ❌ | 6-100字符，不填则不修改 | `"newpassword"` |
| nickname | String | ❌ | - | `"张三(更新)"` |
| email | String | ❌ | - | `"newemail@example.com"` |
| phone | String | ❌ | - | `"13900139000"` |
| userType | String | ❌ | admin/user/service | `"user"` |
| deptId | Long | ❌ | - | `2` |

**curl 示例**

```bash
curl -X PUT http://localhost:9001/api/users/2 \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer <TOKEN>" \
  -d '{"nickname":"张三丰","email":"zhangsanfeng@example.com","deptId":2}'
```

**成功响应**: 同 3.1.4 新增用户响应格式
...
```
GET /api/users/{id}/roles                → 查询用户角色编码列表  [权限: system:user:list]
...
> ⚠️ 返回的是角色编码 `List<String>`，不是 `List<RoleInfo>`。前端分配角色时需将编码匹配到 `getRoles()` 列表中的角色ID。
...
```
PUT /api/users/batch/roles                → 批量分配角色          [权限: system:user:edit]
```

> 🆕 将指定角色批量下发给多个用户，增量添加（不清除已有角色）。适用于权限批量下发场景。

**请求参数 `BatchAssignRoleRequest`**

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| userIds | List\<Long\> | ✅ | 目标用户ID列表 |
| roleIds | List\<Long\> | ✅ | 要分配的角色ID列表 |
...
> 🆕 支持独立勾选父目录和子按钮。勾选父目录 = 仅查看该菜单模块；勾选子按钮 = 具备增删改等操作权限。前后端均为精准 ID 列表模式。
...
```
GET /api/menus/tree                       → 获取完整菜单树（支持筛选）[权限: system:menu:list]
...
> 🆕 `GET /api/menus/tree` 支持查询参数筛选：`?keyword=用户&menuType=1&enabled=true`，参数均为可选，不传返回全部并构建树。
...
```
GET /api/depts/tree                       → 查询部门树（支持筛选） [权限: system:dept:list]
GET /api/depts                            → 查询所有部门（扁平列表） [权限: system:dept:list]
GET /api/depts/{id}                       → 按ID查询部门           [权限: system:dept:list]
POST /api/depts                           → 新增部门               [权限: system:dept:add]
PUT /api/depts/{id}                       → 编辑部门               [权限: system:dept:edit]
PUT /api/depts/{id}/enable                → 启用部门               [权限: system:dept:edit]
PUT /api/depts/{id}/disable               → 禁用部门               [权限: system:dept:edit]
DELETE /api/depts/{id}                    → 删除部门               [权限: system:dept:delete]
```

> 🆕 `GET /api/depts/tree` 支持查询参数筛选：`?keyword=技术&enabled=true`，keyword 匹配部门名称/编码/负责人。

**创建/更新请求 `DeptRequest`**

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| parentId | Long | ❌ | 父部门ID（0=顶级） |
| deptName | String | ✅ | 部门名称 |
...
| sortOrder | Integer | 排序号 |
| enabled | Boolean | 是否启用 |
| children | List\<DeptResponse\> | 子部门列表（树形） |
| createTime | String (ISO) | 创建时间 |
| updateTime | String (ISO) | 更新时间 |
...
/** 后端分页响应: records(数据列表) / total(总数) / page(页码) / pageSize(每页条数) */
...
// 用户分页查询参数
interface UserPageQuery extends PageQuery {
  keyword?: string       // 模糊搜索: 用户名/昵称/邮箱
  enabled?: boolean      // 状态: true=启用 false=禁用
  userType?: string      // 类型: admin/user/service
  deptId?: number        // 部门ID
}

// 用户完整信息 (对齐后端 UserResponse)
interface UserInfo {
  id: number
  username: string
  nickname: string
...
import type { ApiResponse, PageResult, UserInfo, UserPageQuery, UserCreateRequest, UserUpdateRequest, CurrentUserResponse, MenuResponse } from '@/types'

const SYSTEM_BASE = '/system-server'  // 通过 Vite proxy → Gateway(8080) → system-server(9001)

// ==================== 当前用户 ====================

export const getCurrentUser = (): Promise<ApiResponse<CurrentUserResponse>> =>
  request.get(`${SYSTEM_BASE}/api/users/current`)

export const getUserMenus = (): Promise<ApiResponse<MenuResponse[]>> =>
  request.get(`${SYSTEM_BASE}/api/users/current/menus`)

// ==================== 用户管理 ====================

/** 查询所有用户(不分页，慎用) */
export const getUsers = (): Promise<ApiResponse<UserInfo[]>> =>
  request.get(`${SYSTEM_BASE}/api/users`)

/** 分页查询用户(支持多条件筛选) */
export const getUsersPage = (params: UserPageQuery): Promise<ApiResponse<PageResult<UserInfo>>> =>
  request.get(`${SYSTEM_BASE}/api/users/page`, { params })

/** 按ID查询用户 */
export const getUserById = (id: number): Promise<ApiResponse<UserInfo>> =>
  request.get(`${SYSTEM_BASE}/api/users/${id}`)

/** 新增用户(管理员) */
export const createUser = (data: UserCreateRequest): Promise<ApiResponse<UserInfo>> =>
  request.post(`${SYSTEM_BASE}/api/users`, data)

/** 修改用户(管理员,仅传需要修改的字段) */
export const updateUser = (id: number, data: UserUpdateRequest): Promise<ApiResponse<UserInfo>> =>
  request.put(`${SYSTEM_BASE}/api/users/${id}`, data)

/** 启用/禁用用户 */
export const toggleUserStatus = (id: number, enabled: boolean): Promise<ApiResponse<void>> =>
  request.put(`${SYSTEM_BASE}/api/users/${id}/${enabled ? 'enable' : 'disable'}`)

/** 删除用户 */
export const deleteUser = (id: number): Promise<ApiResponse<void>> =>
  request.delete(`${SYSTEM_BASE}/api/users/${id}`)

/** 查询用户角色 */
export const getUserRoles = (id: number): Promise<ApiResponse<RoleInfo[]>> =>
  request.get(`${SYSTEM_BASE}/api/users/${id}/roles`)

/** 分配用户角色 */
export const assignUserRoles = (id: number, roleIds: number[]): Promise<ApiResponse<void>> =>
  request.put(`${SYSTEM_BASE}/api/users/${id}/roles`, { roleIds })
```

### 9.5 Vue3 组件中调用分页接口（完整示例）

```vue
<script setup lang="ts">
import { ref, reactive, onMounted } from 'vue'
import { getUsersPage, toggleUserStatus, deleteUser, createUser, updateUser } from '@/api/system'
import type { UserInfo, UserPageQuery, UserCreateRequest, UserUpdateRequest } from '@/types'
...
// 更新用户
async function handleUpdate(id: number, data: UserUpdateRequest) {
...
<template>
  <div>
    <!-- 搜索栏 -->
    <el-input v-model="searchForm.keyword" placeholder="用户名/昵称/邮箱" @keyup.enter="handleSearch" />
    <el-select v-model="searchForm.enabled" placeholder="状态" clearable @change="handleSearch">
      <el-option label="启用" :value="true" />
      <el-option label="禁用" :value="false" />
    </el-select>
    <el-select v-model="searchForm.userType" placeholder="类型" clearable @change="handleSearch">
